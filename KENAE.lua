...
if text:match("^ارفع منشئ @(.*)$")  then
local apow = {string.match(text, "^(ارفع منشئ) @(.*)$")}
...
if text:match("^ارفع منشئ (%d+)$") then
local apow = {string.match(text, "^(ارفع منشئ) (%d+)$")}
database:sadd('KENAE:'..bot_id..'creator:'..msg.chat_id_, apow[2])
  tsX000(apow[2],msg,"☑┇تم رفع منشئ في البوت")
end
if text:match("^انزل منشئ$") and msg.reply_to_message_id_ then
...
if text:match("^انزل منشئ @(.*)$") then
local apow = {string.match(text, "^(انزل منشئ) @(.*)$")}
local hash =  'KENAE:'..bot_id..'creator:'..msg.chat_id_
...
if text:match("^انزل منشئ (%d+)$") then
local hash =  'KENAE:'..bot_id..'creator:'..msg.chat_id_
...
if text:match("^الكروبات$") or text:match("^$") then
local gps = database:scard('KENAE:'..bot_id.."groups") or 0
...
if (msg.content_.text_ == '2009 فاطمه حسين ' ) then
...
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if text == 'تفعيل متاح'then
database:set("add"..bot_id, "yes")
send(msg.chat_id_, msg.id_, 1, "📛┇تم تفعيل البوت متاح ", 1, 'html')
end
if (text == 'تعطيل متاح') then
database:del("add"..bot_id)
send(msg.chat_id_, msg.id_, 1, "📛┇تم تعطيل البوت متاح", 1, 'html')
end
...
function res_by_username(extra, result, success)
  ...
  if text:match("^مسح تقيد$") and is_mod(msg) and msg.reply_to_message_id_ then
  function res_by_reply(extra, result, success)
 local hash =  'KENAE:'..bot_id..'res:'..msg.chat_id_
  if not database:sismember(hash, result.sender_user_id_) then
  ...
  if text:match("^مسح تقيد @(.*)$") and is_mod(msg) then
  local hash =  'KENAE:'..bot_id..'res:'..msg.chat_id_
  local res = {string.match(text, "^(مسح تقيد) @(.*)$")}
  function res_by_username(extra, result, success)
  if result.id_ then
  HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. result.id_ .. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_pae_previews=True")
  database:srem(hash, result.id_)
  texts = '👤 ⫶ ايديك ⌁ '..msg.sender_user_id_..'\n👞 ⫶ تم الغاء تقيد  العضو '
  else
  texts = '✖️┇خطاء'
  ...
  if text:match("^مسح تقيد (%d+)$") and is_mod(msg) then
  local hash =  'KENAE:'..bot_id..'res:'..msg.chat_id_
  local res = {string.match(text, "^(مسح تقيد) (%d+)$")}
  ...
  if text:match("^ارفع ادمن$")  and is_owner(msg) and msg.reply_to_message_id_ then
  ...
  if text:match("^ارفع ادمن @(.*)$") and is_owner(msg) then
  local apmd = {string.match(text, "^(ارفع ادمن) @(.*)$")}
  function promote_by_username(extra, result, success)
  if result.id_ then
  database:sadd('KENAE:'..bot_id..'mods:'..msg.chat_id_, result.id_)
  texts = '‏🎃𖡻العضو ‏➥  ['..result.title_..'](t.me/'..(apmd[2] or 'KENAEteam')..')\n‏🚸𖡻تم رفعه ادمن'
  else
  texts = '📛┇خطاء'
  end
  send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
  ...
  if text:match("^ارفع ادمن (%d+)$") and is_owner(msg) then
  local apmd = {string.match(text, "^(ارفع ادمن) (%d+)$")}
  database:sadd('KENAE:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
  tsX000(apmd[2],msg,"📛┇تم رفعه ادمن")
  end
  if text:match("^انزل ادمن$") and is_owner(msg) and msg.reply_to_message_id_ then
  ...
  if text:match("^انزل ادمن @(.*)$") and is_owner(msg) then
  local hash =  'KENAE:'..bot_id..'mods:'..msg.chat_id_
  local apmd = {string.match(text, "^(انزل ادمن) @(.*)$")}
  ...
  if text:match("^انزل ادمن (%d+)$") and is_owner(msg) then
  local hash =  'KENAE:'..bot_id..'mods:'..msg.chat_id_
  local apmd = {string.match(text, "^(انزل ادمن) (%d+)$")}
...
tsX000("prore",msg,"👻┇بالفعل تم رفعه عضو مميز") 
  ...
  local text = text:gsub('ارفع مميز','رفع عضو مميز')
  if text:match("^ارفع مميز @(.*)$") and is_owner(msg) then
  local apmd = {string.match(text, "^(ارفع مميز) @(.*)$")}
  function promote_by_username(extra, result, success)
  if result.id_ then
  ...
  if text and text == "" and is_owner(msg) then
...
tsX000(apsi[2],msg,"🚫┇تم كتمه من البوت")
    ...
    if text:match("^مسح كتم @(.*)$") and is_mod(msg) then
    local apsi = {string.match(text, "^(مسح كتم) @(.*)$")}
   ...
   texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'KENAEteam')..')\n🚸┇تم الغاء كتمه من البوت'
    ...
    if text:match("^مسح كتم (%d+)$") and is_mod(msg) then
    local apsi = {string.match(text, "^(مسح كتم) (%d+)$")}
  ...
  send(msg.chat_id_, msg.id_, 1, '👤┇لا تستطيع طرد \n🚸┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    if text:match("^ارفع مدير$") and is_creator(msg) and msg.reply_to_message_id_ then
    ...
    if text:match("^ارفع مدير @(.*)$") and is_creator(msg) then
    local apow = {string.match(text, "^(ارفع مدير) @(.*)$")}
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'KENAEteam')..')\n📛┇تم رفع مدير في البوت'
    ...
    if text:match("^ارفع مدير (%d+)$") and is_creator(msg) then
    local apow = {string.match(text, "^(ارفع مدير) (%d+)$")}
      database:sadd('KENAE:'..bot_id..'owners:'..msg.chat_id_, apow[2])
	    tsX000(apow[2],msg,"👻┇تم رفع مدير في البوت")
  end
    
    if text:match("^انزل مدير$") and is_creator(msg) and msg.reply_to_message_id_ then
    ...
    if text:match("^انزل مدير @(.*)$") and is_creator(msg) then
    local apow = {string.match(text, "^(انزل مدير) @(.*)$")}
    local hash =  'KENAE:'..bot_id..'owners:'..msg.chat_id_
...
local ph = pronumb[2] - 1
  ...
  if (text:match("^$") or text:match("^البوتات$")) and is_mod(msg) then
     local txt = {string.match(text, "^ اظهار البوتات$")}
...
add = (tonumber(database:get('KENAE:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
  send(msg.chat_id_, msg.id_, 1, "‏🎟| عدد جهاتك المظافه ❁➣ *{"..add.."}*\n🚸| سيقوم البوت بحذفها", 1, 'md')
database:del('KENAE:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
  end
  if text:match("^(عدد الاخطاء)$") or text:match("^(اخطائي)$") then
  local edit = database:get('KENAE:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
  send(msg.chat_id_, msg.id_, 1, "🚸┇عدد اخطائك ~⪼ *{"..edit.."}*", 1, 'md')
  end
  ...
  if text:match("^مسح قائمه المحظورين$") and is_creator(msg) then
  local txt = {string.match(text, "^(مسح قائمه المحظورين)$")}
  local function cb(extra,result,success)
  local list = result.members_
  for i=0 , #list do
...
local ex = database:ttl( 'KENAE:'..bot_id.."charge:"..msg.chat_id_)
  ...
  if (text and text == '') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
...
if text == 'هلو' then
moody = "•- ٰ̲ھہلٍُـّٰ̐ہﯛ૭ཻاتـٰۧﮧ ⁽🕸🦊↡₎"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'ارسلان' then
moody = "المطور مالي العشق"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'شلونكم' then
moody = "• - تٰـہٰٖمٰـہٰٖاٰمٰـہٰٖ ¦🌟😺₎⇣۶ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'شلونك' then
moody = "•- اٰلـہٰٖحٰـہٰٖمٰـہٰٖدٰ لـہٰٖلـہٰٖهٰـہٰٖ بٰـہٰٖخٰـہٰٖيٰـہٰٖرٰ ║🐌🕸✤"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'تمام' then
moody = "•دﯛ̲୭مِٰـﮧِۢ يِٰـﮧِۢآربِٰـِﮧۢ ₎┋💗😻"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'هلاو' then
moody = "•ٰ̲ھہلوِآتَہَٰ حہٰٰبّہيِٰہ  ꪲꪳ♚⚡️ֆ⎝"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == '😐' then
moody = "•هٳ شہمْضيہعہُ لہتہصًفُہنٍ 💜💭℡ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'هاي' then
moody = "• هَٰہۧـاَٰيَٰـﮧﮧاَٰتّٰ يَٰـرِٰوٍّحٌٰـہيَٰ🌚💕"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'بوت' then
moody = "•عِٰـﮧِۢيِٰـﮧِۢﯛ̲୭نِٰـﮧِۢۿۿہ •|• ✨🌸‘"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'اريد اكبل' then
moody = "•همزين كاتلك/ج الوصخ 😹💜ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'لتزحف' then
moody = "•خاب كوم/ي لوكي/هہ  😾🌈ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'كلخرا' then
moody = "• خّٰـﮧرِٰاَٰ يَٰتّٰـہرِٰسٌٍ حٌٰلْٰكٍٰـﮧكٍٰ ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'زاحف' then
moody = "• تفكير وصخ مثل وجهك/هو انت /ي كضيت/ي عمرك/ج تركض وره الناس😾😹ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'دي' then
moody = "• خّٰلْٰيَٰنٍٰـﮧيَٰ احترمك 😾ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'فرخ' then
moody = "• لشبهني بيك يادوده😾😹ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
...
if text == 'اكرهك' then
moody = "•هسه عاد اني الاحبك/ج يلا دي🙎🏼‍♂🖤ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'احبك' then
moody = "•اٰوٰفٰـہٰٖ اٰمٰـہٰٖوٰعٰـہٰٖ ֆ🙈💜❙✰ ﴾ 😻👅ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 'باي' then
moody = "• وين مولي/ه🙁💔ֆ"
...
if text == 'واكف' then
moody = "•باقي واتمدد لو تموت مااوكف 😐😒ֆ"
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
end
if text == 